...
export async function getWeatherData(lat: number, lon: number): Promise<WeatherData | null> {
  const params = new URLSearchParams({
    ...
    hourly: 'temperature_2m,relative_humidity_2m,apparent_temperature,precipitation_probability,precipitation,rain,showers,snowfall,weather_code,surface_pressure,cloud_cover,visibility,wind_speed_10m,wind_direction_10m,wind_gusts_10m,uv_index,soil_temperature_0cm,soil_moisture_0_1cm,pressure_msl,is_day', // Added is_day to hourly
    ...
    timezone: 'auto',
    forecast_days: '7', // Ensure we get 7 days for daily and hourly
  });
